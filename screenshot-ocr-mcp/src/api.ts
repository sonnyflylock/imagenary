...
async function extractWithGemini(imageBase64: string): Promise<string> {
...
async function extractWithClaude(imageBase64: string): Promise<string> {
...
// GPT-5 API (OpenAI)
async function extractWithGPT5(imageBase64: string): Promise<string> {
  const apiKey = getApiKey("gpt5");
  if (!apiKey) {
    throw new Error(
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model: "gpt-4o", // Using gpt-4o as GPT-5 may not be released yet
      max_tokens: 4096,
      messages: [
        {
          role: "user",
...
export async function testApiConnection(
  ...
  try {
    switch (provider) {
      ...
      case "claude": {
        // Claude doesn't have a simple test endpoint, validate key format
        if (!apiKey.startsWith("sk-ant-")) {
          return { success: false, error: "Invalid API key format" };
        }
        return {
          success: true,
          message: "Key format valid (connection test skipped)",
        };
      }
